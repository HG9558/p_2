<template>
  <el-form :inline="true" ref="validateForm" :rules="rules" :model="formInline" class="demo-form-inline" v-bind="$attrs" @submit.native.prevent="()=>{}" v-show='props.FormDataList.length'>
    <el-form-item v-for="(item, i) in FormDataList" :key="i" :label="t(item.label ? item.label : '')" :prop="item.model">
      ...
      <DecimalData v-if="item.type === 'decimalInput'" v-model:min="formInline[item.model]" v-model:max="formInline[item.max]"> </DecimalData>
      <CompositeCompontent
      ...
      <el-input
        ...
        @change="changeInput(item.model, formInline[item.model])"
      ...
      >
        <el-option v-for="(selectItem, selectIndex) in item.options" :key="selectIndex" :label="selectItem.label" :value="selectItem.value">
          <img v-if="item.isImg" style="height: 100%; float: left; margin-right: 20px" :src="selectItem.img" /><span>{{ selectItem.label }}</span>
        </el-option>
      </el-select>
      <!-- RADIO -->
      ...
      <el-tooltip
        ...
        placement="top"
        v-if="item.modelType === 'tooltip'"
      >
        <div class="questionBox">
          <span class="questionFilled">?</span>
        </div>
      </el-tooltip>
    </el-form-item>
    <el-form-item>
      <el-button type="primary" @click="submitForm(true)">{{ t('global.query') }}</el-button>
...
import DecimalData from '../decimalData/index.vue';
...
export default defineComponent({
  ...
  emits: ['onSubmit', 'update:modelValue', 'update:jvbObject','onReset'],
  setup(props, ctx) {
    ...
    const date_list = ref<string[]>(['date']);
    ...
    // 去掉空格
    const changeInput = (typeName: any, val: string) => {
      let str = val.trim();
      formInline.value[typeName] = str;
    };

    const modelObjectv: any = computed({
    ...
    const submitForm = (isSerch: boolean = false) => {
     ...
     */
    function setSerchFormData(key: string, val: any, isSubmit: boolean = false) {
