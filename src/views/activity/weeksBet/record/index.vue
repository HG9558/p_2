<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryFormEl" size="small" :inline="true" label-width="auto">
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini"
          @click="handleQuery">{{ t('global.search') }}</el-button>
    ...
    <el-table :data="listData">
      ...
      <el-table-column :label="t('routes.activity.valid_bet_amount') + '(R$)'" align="center" prop="valid_bet_amount" />
      <el-table-column :label="t('routes.activity.wait_bonus_amount') + '(R$)'" align="center" prop="wait_bonus_amount" />
      <el-table-column :label="t('routes.activity.bonus_amount') + '(R$)'" align="center" prop="bonus_amount" />
      <el-table-column :label="t('global.state')" align="center" prop="state" :formatter="(row,cell,cellval)=>stateFormat(cellval)"/>
      ...
      <el-table-column prop="pay_att" :label="t('routes.activity.pay_at')" align="center" width="180">
        <template #default="scope">
          {{ $filters.format(scope.row.pay_at, false) }}
        </template>
      </el-table-column>
      <el-table-column :label="t('global.remark_txt')" align="center" prop="remark" />
...
import { getWeeksRecord } from '@/http/modules/activity'
...
export default defineComponent({
  ...
  setup() {
    ...
    const getList = () => {
      loading.value = true;
      getWeeksRecord({ ...empty.preProcessData(queryParams) }).then((res: any) => {
        queryParams.page === 1 && (total.value = res.data.t);
    ...
    const stateFormat = (val:any)=>{
        return val===1?t('routes.activity.state.1'):
              val===2?t('routes.activity.state.2'):
              val===3?t('routes.activity.state.3'):''
    }
    // 
    getList();
    // 
    return {
      ...
      queryFormEl,
      t, getList,
      resetQuery, handleQuery,stateFormat
    };
  }
});
</script>
