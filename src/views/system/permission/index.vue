<template>
  <div class="pg-view permission-view">
    <section class="oper-row">
      <el-button type="primary" @click="operatHandler('add')">{{
        t('global.increment')
      }}</el-button>
    ...
    <section class="flex">
      <div class="role-tree-wp">
        <el-tree
          :data="roleAuthTree"
          :props="roleAuthTreeDefaultProps"
          @node-click="roleAuthClickHandler"
          empty-text=""
        ></el-tree>
      </div>
      <div class="flex1 role-per-sec">
        <el-form :label-position="labelPosition" label-width="auto" :model="rolePermissionForm">
          ...
          <el-form-item :label="t('routes.system.module_name')">
            <el-input
              v-model="rolePermissionForm.name"
              :placeholder="t('routes.system.module_tips1')"
              :disabled="roleAuthFormCanEdit"
            ></el-input>
          </el-form-item>
          <el-form-item :label="t('routes.system.module_router')">
            <el-input
              v-model="rolePermissionForm.module"
              :placeholder="t('routes.system.module_tips2')"
              :disabled="roleAuthFormCanEdit"
            ></el-input>
          </el-form-item>
          <el-form-item :label="t('routes.system.module_up')" class="role-p-set">
            ...
            <el-button type="primary" @click="setting" :disabled="!isAdd">{{
              t('routes.system.pz')
            }}</el-button>
          </el-form-item>
          <el-form-item :label="t('routes.system.module_state')">
            <el-radio
              v-model="rolePermissionForm.state"
              :label="0"
              :disabled="roleAuthFormCanEdit"
              >{{ t('routes.system.close') }}</el-radio
            >
            <el-radio
              v-model="rolePermissionForm.state"
              :label="1"
              :disabled="roleAuthFormCanEdit"
              >{{ t('routes.system.open') }}</el-radio
            >
          </el-form-item>
    ...
    <el-dialog :title="t('routes.system.role_moudel')" v-model="dialogFormVisible" width="800px" :close-on-click-modal="false">
      <!-- <header>
          <span>权限名称</span>
      ...
      <div class="dialog-role-tree-wp">
        ...
        >
          <template v-slot:default="slotProps">
            <el-radio v-model="dialogRoleTreeItemRadio" :label="slotProps.data.id">{{
              slotProps.data.name
            }}</el-radio>
          </template>
        </el-tree>
