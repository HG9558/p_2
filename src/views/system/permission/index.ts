...
export default defineComponent({
  ...
  setup() {
    ...
    const roleAuthClickHandler = (data: typeof ElTree) => {
      const item: ResponsePriveListItem | undefined = getPriveItemByPid(
        data.pid,
        storePriveList
      );
      if (isAdd.value) {
    ...
    const dialogRoleAuthClickHandler = (data: typeof ElTree) => {
      cachedDialogRoleTreeSelected = Object.assign(
        {},
        formeDatas.rolePermissionForm,
        { pname: data.name, pid: data.id, state: data.state }
      );
      ...
      switch (type) {
        ...
        case 'edit':
          if (formeDatas.rolePermissionForm.id === -1) {
            app?.appContext.config.globalProperties.$message.warning(
              '请选择列表中的一项'
            );
            return;
          }
    ...
    const modify = () => {
      ...
      privUpdate(params).then((res: any) => {
        if (res.status) {
          initData();
          app?.appContext.config.globalProperties.$message.success('修改成功');
        }
      });
    };
