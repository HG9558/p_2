    ...
    <div class="betting-cont">
      <div class="betting-table">
        ...
        <el-table :data="tableData" border style="width: 100%" :max-height="globalTableHeight">
          ...
          <el-table-column prop="phone" :label="t('routes.vip_list_page.vip_usephone')" align="center"> </el-table-column>
          <el-table-column prop="code" :label="t('routes.vip_list_page.phone_code')" align="center">
...
import {accountType } from '@/utils/requestOptions';
...
export default defineComponent({
  ...
  setup() {
    ...
    function getList() {
      ...
      APIs.codequery.verify(params).then(res => {
        tableData.value = [];
        if (res.status) {
    ...
    const getListItem = (page: number) => {
      return new Promise((resolve, reject) => {
        ...
        APIs.codequery.verify(obj).then((res: any) => {
          if (res.status) {
            resolve(res.data.d ?? []);
          } else {
            resolve([]);
    ...
    const exportHanlder = () => {
      ...
      const total_page = Math.ceil(totalCount.value / 100);
      ...
      Promise.all(PromiseList).then(res => {
        ...
        const data = result.map((item: any) => {
          return {
            ty: qudaoType[+item.ty].label,
            username: item.username,
            phone: item.phone,
            code: item.code,
            flags: item.flags,
            created_at: format(+item.create_at + 0, false),
            outTime: item.state == 1 ? format(+item.create_at + 10 * 60, false) : (item.state == 2 ? '-' : ''),
            source: item.source,
            ip: item.ip,
        ...
        const fields: any = [
          { label: t('global.ty'), value: 'ty' },
          { label: t('global.username'), value: 'username' },
          { label: t('routes.vip_list_page.vip_usephone'), value: 'phone' },
          { label: t('routes.vip_list_page.phone_code'), value: 'code' },
          { label: t('routes.vip_list_page.vip_type'), value: 'flags' },
          { label: t('global.send_time'), value: 'created_at' },
          { label: t('routes.tips.cgsj'), value: 'outTime' },
          { label: t('routes.vip_list_page.vip_source'), value: 'source' },
          { label: 'IP', value: 'ip' },
